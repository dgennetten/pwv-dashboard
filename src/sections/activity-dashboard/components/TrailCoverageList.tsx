...
import type { TrailCoverageRow, TrailCoverageSortKey } from '@/../product/sections/activity-dashboard/types'
...
interface TrailCoverageListProps {
  ...
  /**
   * Rows per page: initial load and each “Load more”. Default 50.
   * Wire from org admin settings in production.
   */
  pageSize?: number
...
export function TrailCoverageList({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        setScrollEndReached(entry.isIntersecting)
      },
      ...
      {showLoadMore && (
        <div className="flex flex-col items-center gap-1 pt-1">
          <button
            ...
            className="text-xs font-medium text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 underline-offset-2 hover:underline"
          >
